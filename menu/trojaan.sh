...
function menu2() {
	...
	export user=Trial$(tr </dev/urandom -dc X-Z0-9 | head -c4)
...
function menu7() {
	...
	export patchtls=/timevpn-trojanwstls
	export patchnone=/timevpn-trojanwsntls
	read -p "   Bug Address (Example: www.google.com) : " address
	read -p "   Bug SNI (Example : m.facebook.com) : " sni
	read -p "   Expired (days) : " masaaktif
...
function menu8() {
	...
	echo "trojanws $user $exp $harini $user $get_AdminID $get_AdminID active" >>"/root/.database"
	#############
...
function menu9() {
	...
	#############
	get_AdminID=$(sed -n '2 p' /root/ResBotAuth | cut -d' ' -f2)
	sed -i "/trojanws $user $exp $harini/d" "/root/.database"
	echo "trojanws $user $exp $harini $user $get_AdminID $get_AdminID inactive" >>"/root/.database"
	#############
	systemctl restart xray@trojannone
	systemctl restart xray@trojan
	service cron restart
...
function menu10() {
	...
	get_AdminID=$(sed -n '2 p' /root/ResBotAuth | cut -d' ' -f2)
	sed -i "/trojanws $user $exp $harini/d" "/root/.database"
	echo "trojanws $user $exp4 $harini $user $get_AdminID $get_AdminID active" >>"/root/.database"
	#############
	service cron restart
