...
# USERNAME
rm -f /usr/bin/user
username=$(curl -s https://raw.githubusercontent.com/${GitUser}/multi-access/main/access | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
# Order ID
rm -f /usr/bin/ver
user=$(curl -s https://raw.githubusercontent.com/${GitUser}/multi-access/main/access | grep $MYIP | awk '{print $3}')
echo "$user" >/usr/bin/ver
# validity
rm -f /usr/bin/e
valid=$(curl -s https://raw.githubusercontent.com/${GitUser}/multi-access/main/access | grep $MYIP | awk '{print $4}')
...
ver=$(curl -s https://raw.githubusercontent.com/${GitUser}/multi-access/main/versi)
clear
# CEK UPDATE
...
version=$(cat /home/ver)
new_version=$(curl -s https://raw.githubusercontent.com/${GitUser}/multi-access/main/versi | grep $version)
#Status Version
...
Exp1=$(curl -s https://raw.githubusercontent.com/${GitUser}/multi-access/main/access | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
    sts="${Info}"
else
    sts="${Error}"
...
echo -e " \e[$line╒════════════════════════════════════════════════════════════╕\e[m"
echo ":::::::Traffic:Today:Yesterday:Month:" >/root/t1
echo ":::::::Download:$dtoday:$dyest:$dmon:" >>/root/t1
echo ":::::::Upload:$utoday:$uyest:$umon:" >>/root/t1
echo ":::::::Total:$ttoday:$tyest:$tmon:" >>/root/t1
column -t -s':' /root/t1
rm -f /root/t1
echo -e " \e[$line╘════════════════════════════════════════════════════════════╛\e[m"
...
echo -e "  \e[$number (•5)\e[m \e[$below CHECK RUNNING\e[m        \e[$number (•9)\e[m \e[$below DYNAMIC IP\e[m"
...
echo -e "  \e[$below Autoscript By        : ARTHARON"
